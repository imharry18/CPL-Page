 ...
 *
 * Two: the running order, then the players nobody wanted. There is no third
 * time of asking — the Recall is the last one, and nobody can be passed over
 * in it (see the "unsold" case in /api/auction), so every player in it is
 * sold and the night ends with the pool empty.
 */
 ...
 * players nobody wanted. Kept beside ROUNDS so raising the cap and naming the
 * new round are one change rather than two — anything past the list falls back
 * to a number rather than going blank. */
const NAMES = ["", "Opening", "Recall"];
 ...
 * In the Recall, the players who went unsold in the Opening and have not been
 * offered since. Nobody can go unsold in the Recall, so there is nothing left
 * for a round after it to call.
 *
...
export function nextInQueue(queue = [], current = null) {
  ...
  if (at >= 0) return queue.find((name, i) => i > at && name !== current) ?? null;
  return queue.find((name) => name !== current) ?? null;
}
